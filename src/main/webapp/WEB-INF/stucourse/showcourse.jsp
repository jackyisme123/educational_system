...
<body>
	...
	<div class="content">
		<div>
			<h1>贵族养成教程</h1>
		<table class="datalist" border="1" width=80% >
          		<tr>
              		<th>课程代码</th>
			 	...
			 	<c:forEach items="${courses }" var="c">
				 	<tr>
				 		...
				 		<td>${c.examDate }</td>
				 		<td>
                        	<input type="button" value="选择"  onclick="location.href='toAddStuCourse.do?id=${c.courseId}';" />
				 		</td>
				 	</tr>
			 	</c:forEach>
			</table>
		...
		<div class="pages">
              ...
              <c:if test="${page!=1 }">
   	        	  <a href="toShowCourse.do?page=${page-1 }">上一页</a>
              </c:if>
              ...
              <c:forEach begin="1" end="${total }" var="i">
              	 <!-- 若循环到了当前页，则将页码高亮显示 -->
              	 <c:if test="${i==page }">
              	 	<a href="toShowCourse.do?page=${i }">${i }</a>
              	 </c:if>
              ...
              <c:if test="${page!=total }">
               <a href="toShowCourse.do?page=${page+1 }">下一页</a>
              </c:if>
        </div>
        <div>
        		<input type="button" value="返回"  onclick="location.href='findStuCourse.do?stuName=${stuName }'" />
        </div>
	</div>
	</div>
